...
function AppLayout() {
  ...
  return (
    <div className="min-h-screen bg-muted/20 flex">
      <aside className="w-60 hidden md:flex flex-col border-r border-border bg-pitch text-primary-foreground">
        <Link to="/" className="px-5 h-16 flex items-center font-black tracking-tight border-b border-primary-foreground/15 text-primary-foreground">CopaHub SaaS</Link>
        <nav className="flex-1 p-3 space-y-1">
          {nav.map(({ to, label, icon: Icon, exact }) => (
            <Link key={to} to={to} activeOptions={{ exact: !!exact }}
              className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-primary-foreground/80 hover:bg-primary-foreground/10 hover:text-primary-foreground"
              activeProps={{ className: "flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-semibold bg-primary-foreground/15 text-primary-foreground" }}>
              <Icon className="h-4 w-4" />{label}
            </Link>
          ))}
        </nav>
        <div className="p-3 border-t border-primary-foreground/15 space-y-1">
          <Link to="/" className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-primary-foreground/80 hover:bg-primary-foreground/10 hover:text-primary-foreground">
            <ExternalLink className="h-4 w-4" /> Ver site público
          </Link>
          <button onClick={logout} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-primary-foreground/80 hover:bg-primary-foreground/10 hover:text-primary-foreground">
