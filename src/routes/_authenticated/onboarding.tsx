...
function Onboarding() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!u.user) return;
      const { data: t } = await supabase.from("tenants").select("id, nome_estabelecimento, whatsapp, cidade, estado").eq("owner_user_id", u.user.id).maybeSingle();
      if (t) {
        setTenantId(t.id);
        setS1((v) => ({ ...v, nome_estabelecimento: t.nome_estabelecimento ?? "", whatsapp: t.whatsapp ?? "", cidade: t.cidade ?? "", estado: t.estado ?? "" }));
        const { data: bo } = await supabase.from("boloes").select("id").eq("tenant_id", t.id).limit(1);
  ...
  async function saveStep2() {
    ...
    try {
      const { error } = await supabase.from("tenant_pix_config").upsert({ tenant_id: tenantId, ...s2 }, { onConflict: "tenant_id" });
      if (error) throw error;
      setStep(3);
      ...
      <div className="mx-auto max-w-2xl px-4">
        <div className="mb-8">
          ...
          <div className="mt-3 flex gap-1">
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className={`h-2 flex-1 rounded-full ${n <= step ? "bg-pitch" : "bg-muted"}`} />
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          {step === 2 && (
            <Form title="Configuração do Pix" onSubmit={saveStep2} loading={loading}>
              <Input label="Nome do recebedor" value={s2.nome_recebedor} onChange={(v) => setS2({ ...s2, nome_recebedor: v })} required />
              <Select label="Tipo de chave" value={s2.tipo_chave_pix} onChange={(v) => setS2({ ...s2, tipo_chave_pix: v })}
                options={[["cpf","CPF"],["cnpj","CNPJ"],["email","E-mail"],["telefone","Telefone"],["aleatoria","Aleatória"]]} />
              <Input label="Chave Pix" value={s2.chave_pix} onChange={(v) => setS2({ ...s2, chave_pix: v })} required />
              <div className="grid grid-cols-2 gap-3">
                <Input label="Banco" value={s2.banco} onChange={(v) => setS2({ ...s2, banco: v })} />
          ...
          {step === 3 && (
            <Form title="Configuração do WhatsApp" onSubmit={saveStep3} loading={loading}>
              ...
              <Textarea label="Mensagem para ganhador" rows={3} value={s3.mensagem_ganhador} onChange={(v) => setS3({ ...s3, mensagem_ganhador: v })} />
              <Textarea label="Mensagem de lembrete de pagamento" rows={3} value={s3.mensagem_lembrete_pagamento} onChange={(v) => setS3({ ...s3, mensagem_lembrete_pagamento: v })} />
            </Form>
          )}
          {step === 4 && (
            <Form title="Criar primeiro bolão" onSubmit={saveStep4} loading={loading}>
              ...
              <Input label="Slug (link público)" value={s4.slug} onChange={(v) => setS4({ ...s4, slug: slugify(v) })} prefix="/bolao/" required />
...
function Form({ title, children, onSubmit, loading }: { title: string; children: React.ReactNode; onSubmit: () => void; loading: boolean }) {
  return (
    <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onSubmit(); }}>
      <h2 className="text-xl font-bold">{title}</h2>
      {children}
      <button type="submit" disabled={loading} className="mt-4 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-pitch px-4 font-semibold text-primary-foreground disabled:opacity-60">
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Check className="h-4 w-4" /> Continuar <ChevronRight className="h-4 w-4" /></>}
      </button>
    </form>
  );
}

function Input(p: { label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean; prefix?: string; placeholder?: string; hint?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>["inputMode"] }) {
  return (
    <label className="block">
...
function Select(p: { label: string; value: string; onChange: (v: string) => void; options: [string, string][] }) {
  return (
    <label className="block">
      <span className="text-sm font-medium">{p.label}</span>
      <select value={p.value} onChange={(e) => p.onChange(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-pitch/40">
