  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {sorted.map((r, i) => {
            const isClassified = r.status_classificacao === "classificado";
            const isEliminated = r.status_classificacao === "eliminado";
            return (
              <tr
                key={r.team_id}
                className={`border-b border-border/60 hover:bg-muted/40 ${isClassified ? "bg-pitch/5" : isEliminated ? "opacity-60" : ""}`}
              >
                <td className="py-2 pl-3">
                  <span
                    className={`inline-grid place-items-center h-6 w-6 rounded-full text-xs font-bold ${i < highlightTop ? "bg-pitch text-primary-foreground" : "bg-muted"}`}
                  >
                    {i + 1}
                  </span>
                </td>
