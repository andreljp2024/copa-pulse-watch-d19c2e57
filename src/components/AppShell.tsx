...
import { Moon, Sun, Menu, X, HelpCircle } from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  // Inicializa tema e email APENAS após hydration (cliente)
  useEffect(() => {
    setMounted(true);
    ...
    // Auth via getUser() (nao usa realtime para evitar hydration issues)
    supabase.auth.getUser().then(({ data }) => setEmail(data.user?.email ?? null));
  }, []);

  // Realtime listener APENAS apos mount
  useEffect(() => {
    if (!mounted) return;
  ...
  if (!mounted) {
    return (
      <div className="min-h-screen flex flex-col relative">
        <div aria-hidden className="fixed inset-0 -z-10 bg-cover bg-center bg-no-repeat opacity-[0.12] pointer-events-none" style={{ backgroundImage: `url(${bgPattern})` }} />
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      <div
        aria-hidden
        className="fixed inset-0 -z-10 bg-gradient-to-b from-background/80 via-background/95 to-background pointer-events-none"
      />
      <header className="sticky top-0 z-40 border-b border-border bg-background/85 backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 h-16 flex items-center gap-4">
          <Link to="/" className="flex items-center gap-2 shrink-0">
          ...
          <nav className="hidden md:flex items-center gap-1 ml-4">
            {nav.map((n) => (
              <Link
                ...
                activeProps={{
                  className: "px-3 py-2 rounded-lg text-sm font-semibold text-foreground bg-muted",
                }}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Link
              to="/criar-bolao"
              className="hidden sm:inline-flex h-9 items-center rounded-lg border border-pitch text-pitch px-3 text-sm font-semibold hover:bg-pitch/5"
            >
              Criar meu bolão
            </Link>
            {email ? (
              <Link
                to="/app"
                className="hidden sm:inline-flex h-9 items-center rounded-lg bg-pitch px-3 text-sm font-semibold text-primary-foreground hover:opacity-90"
              >
                Meu painel
              </Link>
            ) : (
          ...
          <div className="md:hidden border-t border-border bg-background">
            <div className="mx-auto max-w-7xl px-4 py-2 flex flex-col">
              ...
              <Link
                to={email ? "/app" : "/auth"}
                onClick={() => setOpen(false)}
                className="px-3 py-3 rounded-lg text-sm font-semibold text-pitch"
              >
