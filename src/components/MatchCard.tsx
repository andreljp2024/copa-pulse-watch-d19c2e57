    ...
    <div className={`flex items-center gap-2 min-w-0 ${className}`}>
      {team.flag_url ? (
        <img
          src={team.flag_url}
          alt={displayName}
          className={`${sz} object-cover rounded-sm shrink-0 ring-1 ring-border`}
          loading="lazy"
        />
      ) : (
        <div
          className={`${sz} rounded-sm bg-muted grid place-items-center text-[10px] font-bold shrink-0`}
        >
          {team.code}
        </div>
      )}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between text-xs text-muted-foreground mb-3">
        ...
        {isFinished && (
          <span className="px-2 py-0.5 rounded-full bg-muted font-semibold">Encerrado</span>
        )}
        {!isLive && !isFinished && (
          <span className="px-2 py-0.5 rounded-full bg-muted">Agendado</span>
        )}
      </div>
